...
use App\Notifications\Frontend\Auth\BrokerContractUploaded;
...
class UserRepository extends BaseRepository
{
    ...
    public function create(array $data)
    {
        return DB::transaction(function () use ($data) {
            $user = parent::create([
                ...
                'mobile_no'             => !empty($data['mobile_no']) ? $data['mobile_no'] : '',
                'company_name' => !empty($data['company_name']) ? $data['company_name'] : '',
                'tax_no' => !empty($data['tax_no']) ? $data['tax_no'] : '',                
     ...
     * @throws GeneralException
     */
    public function update($id, array $input, $image = false, $broker_contract = false)
    {
        ...
        $user->commission_sale_landlord = array_get($input, 'commission_sale_landlord');
        $user->commission_sale_buyer = array_get($input, 'commission_sale_buyer');
        $user->commission_rent_landlord = array_get($input, 'commission_rent_landlord');
        $user->commission_rent_tenant = array_get($input, 'commission_rent_tenant');

		$administrator = User::role('administrator')->get();
        $done_by = auth()->user();
        
        ## Uploading Broker contract_file
        if ($broker_contract) {
            $user->contract_file = $broker_contract->store(config('app.broker_contracts_file_path'));

            ## Notification
            if ($user->contract_file) {
                \Notification::send($administrator, new BrokerContractUploaded($done_by, $user));

                if (!empty($user)) {
                    \Notification::send($user, new BrokerContractUploaded($done_by, $user));
                }
            }
        }
