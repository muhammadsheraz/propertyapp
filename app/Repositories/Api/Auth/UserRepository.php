...
class UserRepository extends BaseRepository
{
    ...
    public function create(array $data)
    {
        return DB::transaction(function () use ($data) {
            $user = parent::create([
                ...
                'other_city'             => !empty($data['other_city']) ? $data['other_city'] : '',
            ...
            if ($user) {
                ## Generating and Saving Customer No 
                $customer_no = 'C' . str_pad($user->id,config('app.customer_no_padding_size'),"0",STR_PAD_LEFT);
                $user->customer_no = $customer_no;
                $user->save();
                /*
                 * Add the customer role
                 */
                $user->assignRole(config('access.users.customer_role'));
            }
    ...
    public function update($id, array $input, $image = false)
    {
        ...
        if  (!empty($input['other_city'])) {
            $user->city_id = $input['other_city'];
        }

        if  (!empty($input['district_id'])) {
            $user->district_id = $input['district_id'];
        }
    ...
    public function updateAvatar($id, $image = false)
    {
        $user = $this->getById($id);
